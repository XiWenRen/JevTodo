import { registerUser, authenticateUser, getUserById, isCloudDBConfigured } from '../server/db.js';
...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    if (req.method === 'POST') {
      ...
      if (action === 'register') {
        ...
        if (!isCloudDBConfigured()) {
          // In local mode without DB, simulate successful guest account
    ...
    return res.status(405).json({ error: 'Method or action not supported' });
  } catch (err: any) {
    console.error('Auth API error:', err);
    return res.status(500).json({ error: err?.message || '身份认证服务异常' });
  }
}
